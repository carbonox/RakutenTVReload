...
public class Pelicula2 implements Serializable {
    ...
    private final static String CINE = "cine";
    ...
    public static ArrayList<Pelicula2> getArrayListFromJSon(JSONArray datos) {
        ArrayList<Pelicula2> lista = null;
        Pelicula2 pelicula = null;
        try {
            if (datos != null && datos.length() > 0) {
                lista = new ArrayList<Pelicula2>();
            }
            for (int i = 0; i < datos.length(); i++) {
                JSONObject json_data = datos.getJSONObject(i);
                pelicula = new Pelicula2();
